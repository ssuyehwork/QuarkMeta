...
void ColorPill::mousePressEvent(QMouseEvent* event) {
    ...
    if (event->button() == Qt::LeftButton) {
        ...
        menu.addAction(UiHelper::getIcon("search", QColor("#FFFFFF"), 18), "搜索相似颜色的项目", [this, color]() { emit colorSelected(color); });
        menu.addSeparator();
        QString hex = color.name().toUpper();
        menu.addAction(UiHelper::getIcon("copy", QColor("#FFFFFF"), 18), QString("复制 %1").arg(hex), [hex]() { QApplication::clipboard()->setText(hex); });
        menu.addSeparator();
        menu.addAction(UiHelper::getIcon("star_filled", QColor("#FFFFFF"), 18), "设置为自定义主色", [this, color]() { emit requestSetAsPrimary(color); });
