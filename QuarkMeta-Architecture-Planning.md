...
1. **对标 Windows Standard 直观交互**：
   抛弃“自动解析”等抽象晦涩的术语，冲突选项必须采用直观、无歧义的标准语言：
   - **替换目标中的文件**：直接覆盖目标文件夹中的同名文件；
   - **跳过这些文件**：保留目标文件夹中的原文件，不进行粘贴/移动；
   - **保留两者（自动重命名）**：同时保留目标文件，新写入的文件按标准递增规则编号重命名（如 `文件 (1).ext`）。
