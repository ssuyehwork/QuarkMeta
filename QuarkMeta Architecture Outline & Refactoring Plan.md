...
为保证代码库的高内聚、低耦合与长期可维护性，全工程建立**防止打补丁的五道工程硬锁机制**：

1. **【契约锁 (Freeze Contracts)】**：`.h` 头文件处于只读/冻结状态，严禁随意修改已有的 `public` 函数签名及参数。扩展必须通过重载或默认参数完成。
2. **【依赖锁 (Unidirectional Flow)】**：严格锁定 $\text{Shell} \to \text{View} \to \text{Controller} \to \text{Domain} \to \text{Data}$ 单向依赖流，严禁逆向包含头文件与跨层下钻。
3. **【门禁锁 (3-Question Gate)】**：所有方案必须率先回答“架构三问”（状态唯一持有者 SSOT、黑盒封装完整性、根因而非表象）。
4. **【静态红线扫描 (Anti-Patch Linting)】**：严禁 `SetForegroundWindow` 强抢焦点、工具类篡改 `Qt::Popup`、`friend class` 伪解耦、`installEventFilter` 无校验全局挂载以及 `for` 循环内高频发射事件。
5. **【黑盒隔离锁 (Information Hiding)】**：仅对接口 (API/DTO) 负责，禁止跨模块窥探或走捷径依赖私有实现。

所有开发与重构行为必须无条件遵守以下 **6 大架构红线**：
